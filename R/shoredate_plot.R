...
shoredate_plot <- function(shorelinedate,
  ...
  hdrdates <- shoredate_hdr(shorelinedate)
  # Visualisation cut-off
  dategrid <- shorelinedate$date[shorelinedate$date$probability > 0.000009,]
  ...
  if(lambda == TRUE){
    plt <- plt +
      ggplot2::ggtitle(paste("\U03BB =", as.numeric(shorelinedate$expratio)))
  }

  if(displacement_curve == TRUE){
    plt <- plt +
      ggplot2::theme(legend.position = "None") +
      ggplot2::geom_ribbon(data = shorelinedate$dispcurve,
                           ggplot2::aes(x = .data$bce,
                                        ymin = .data$lowerelev,
                                        ymax = .data$upperelev),
                           fill = "red", alpha = 0.2) +
      ggplot2::geom_line(data = shorelinedate$dispcurve,
                         ggplot2::aes(x = .data$bce,
      ...
      ggplot2:: coord_cartesian(ylim = c(0,
                                         as.numeric(shorelinedate$elev) + 30))
  }

  if(site_elevation == TRUE){

  #  For plotting purposes to close the geom_polygon on the y-axis
  expdatg <- rbind(c(0, 0, 0), shorelinedate$expdat)

  # Code taken from oxcAAR to plot density to y-axis
  x_extent <- ggplot2::ggplot_build(plt)$layout$panel_scales_x[[1]]$range$range
  ...
  plt <- plt + ggplot2::geom_polygon(data = expdatg,
                                  ...
                                  y = as.numeric(shorelinedate$elev) -
                                    .data$offset),
                           fill = "#046c9a", alpha = 0.6) +
  ggplot2::geom_hline(yintercept = as.numeric(shorelinedate$elev),
                      linetype = "dashed",
  ...
  if(hdr_label == TRUE){
    ...
    grob <- grid::grobTree(grid::textGrob(label_text, x = 0.83,
                                          y = 0.87, hjust = 0,
