...
#'
#' @return A data frame holding median, start and end points for segments of the 95% highest density region
#' @export
#'
#' @import hdrcde
#'
...
shoredate_hdr <- function(shorelinedate){
  ...
  datedat$group <- seq(1:length(segdat$hdr[c(TRUE, FALSE)]))
  datedat$year_median <- stats::median(dathdr$mode)
