...
#'
#' Function to find 95% highest density region (HDR) for a provided shoreline date rounded to the nearest 10 years to reduce spurious precision.
#'
...
shoredate_hdr <- function(shorelinedate){
  ...
  datedat <- data.frame(matrix(nrow = length(rsegdat[seq(1, nrow(rsegdat), 2),]),
                                ncol = 4))
  names(datedat) <- c("start", "end", "group", "year_median")
  datedat$start <- rsegdat[seq(1, nrow(rsegdat), 2), "X95."]
  datedat$end <- rsegdat[seq(2, nrow(rsegdat), 2), "X95."]
