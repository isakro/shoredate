...
<img src="../man/figures/coverage.png" alt="plot of chunk limit" width="100%" style="display: block; margin: auto;" />
...
When a `SpatRaster` has been loaded into the R session this can be passed to `shoreline_date()` with the `elevation` argument to find site elevations without having to provide this manually, as was done above. Note that if the site is represented by an object with a spatial extent (i.e. not a point feature) the default is to use the mean of the elevation on the raster, but this can be changed with the `elev_fun` argument. Any function accepted by the `fun` argument in `terra::extract()` can be used (see `?terra::extract`).


```r
raster_date <- shoreline_date(target_point, elevation = elevation)
...
iso_date <- shoreline_date(site = target_point, elevevation = elevation, isobase_direction = c(325, 338))
```

```
## Error in shoreline_date(site = target_point, elevevation = elevation, : unused argument (elevevation = elevation)
```
...
```
## Error in eval(expr, envir, enclos): object 'iso_date' not found
```
...
```
## Error in shoredate_plot(iso_date, isobase_direction = TRUE): object 'iso_date' not found
```
...
```r
sum_iso_date <- shoreline_date(site = target_point, elevation = elevation,
