---
title: "Shoreline dating Stone Age sites on the Norwegian Skagerrak coast using the shoredate package"
...
vignette: >
  %\VignetteIndexEntry{Shoreline dating Stone Age sites on the Norwegian Skagerrak coast using the shoredate package}
...
To shoreline date a site the site has to be provided as a spatial object in the *simple feature* format from the `sf` package. In a addition to this, the elevation of the site above the present sea-level has to be provided. This can either be done by manually specifying the elevation, or by providing an elevation raster in the *SpatRaster* object from the `terra` package. The elevation of the site will then be derived from the raster. If the site has a spatial extent (i.e. is not a point feature) the mean elevation of the feature is used.
...
```{r plot_discurves, fig.width = 6, fig.height = 4, warning = FALSE}
displacement_plot()
```

As the degree of isostatic uplift is higher along a gradient than runs from the south west to the north east, the shoreline displacement curve for any given location is interpolated using the `interpolate_curve()` command. At present, this is only done using inverse distance weighting

```{r interpolate, fig.width = 6, fig.height = 4, warning = FALSE}
target_curve <- interpolate_curve(target_point)
```

This visualise the interpolated curve alongside the geologic reconstructions, the curve can be passed to `displacement_plot()`:

```{r plot_intcurve, fig.width = 4.5, fig.height = 3.5, warning = FALSE, message = FALSE}
displacement_plot(target_curve)
```

## Direction of uplift gradient
The precise direction of the uplift gradient is specified by isobases that run perpendicular to the gradient. When interpolating the displacement curve used for dating a site, the direction of these isobases default to 327. However, it is possible to specify other and multiple directions for these, and perform the shoreline date using each individual direction. 

```{r example_site_isobases, message = FALSE}
# Using the same target point and elevation as above, 
# but specifying two different directions for the isobases
