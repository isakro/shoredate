...
```{r limit, echo = FALSE, out.width = '100%'}
# Include precompiled figure (uses more spatial data than what is included in the package)
knitr::include_graphics("../man/figures/coverage.png")
```
...
```{r plot_location, fig.width = 4.3, fig.height = 3.5}
target_plot(target_point)
```

```{r plot_intcurve, fig.width = 5.7, fig.height = 4}
...
```{r plot_example, fig.width = 4.4, fig.height = 3.7}
...
```{r greyscale, fig.width = 4, fig.height = 3.7}
...
```{r multipoint, fig.width = 4.3, fig.height = 3.5}
