...
Furthermore, as human occupation in the region only occurred some time after the retreat of the Fennoscandian Ice Sheet, the oldest known sites are dated to around 9300 BCE (e.g. Glørstad 2016). The oldest possible age to achieve with *shoredate* is 9469 BCE, although no sites are yet known to be that old. 9469 BCE is the latest start date among the geologically derived displacement curves. If a site has an elevation that implies a date older than 9469 BCE, the date is returned as NA and a warning is given.
