...
Shoreline dating of Stone Age sites in northern Scandinavia is based on the premise that coastal sites within the region tend to have been located on or close to the shoreline while they were in use. By reconstructing the past trajectory of shoreline displacement and combining this with the present-day elevation of a site, this thus allows us to ascribe an approximate date to when the sites were in use. The method is therefore dependent on local relative sea-level change and the likely elevation of the sites above sea-level when they were in use.

In the paper Roalkvam (2022) it was found that the vertical distance between sites and the contemporaneous shoreline within the study region could be reasonably approximated by a gamma distribution. This forms the foundation for the implemented method. Furthermore, the method is here based on interpolating the shoreline displacement to any one location within the study area, using the four available geologically derived shoreline displacement curves from the region. This is done in the function `interpolate_curve`, by means of inverse distance weighting, using the squared inverse of the distance between the sites and the shoreline isobases of the displacement curves. The isobases are contours on which relative sea-level change has followed the same trajectory.
...
```{r sparse, fig.width = 4, fig.height = 3.5, warning = FALSE}
...
```{r plot_intcurve, fig.width = 6, fig.height = 5, warning = FALSE, message = FALSE}
...
```{r plot_example_isobases, fig.width = 6, fig.height = 10, warning = FALSE, message = FALSE}
